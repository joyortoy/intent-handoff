...
describe("handoff message", () => {
  it("tells the AI to read page context instead of asking the human again", () => {
    const message = buildHandoffMessage("confirm_123", 3);
    ...
    expect(message).toContain("I selected 3 ordered instructions");
    expect(message).toContain("Follow handoffSteps in order");
  });
});
